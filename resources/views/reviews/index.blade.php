    ...
    <div class="min-h-screen bg-gradient-to-b from-gray-900 via-purple-900 to-black text-white">
        ...
        <div class="relative py-20 px-4 hero-bg">
            ...
            <div class="relative max-w-6xl mx-auto text-center">
                <h1
                    class="text-5xl md:text-7xl font-bold mb-6 text-transparent bg-clip-text bg-gradient-to-r from-purple-400 to-red-600">
                    {{ $type ? ucfirst($type === 'movie' ? 'Filmes' : ($type === 'book' ? 'Livros' : 'Séries')) : 'Dicas e Avaliações' }}
                </h1>
                ...
                @if (auth()->check())
                    <a href="{{ route('reviews.create') }}"
                        class="inline-flex items-center gap-2 bg-gradient-to-r from-red-600 to-purple-600 text-white px-6 py-3 rounded-lg hover:from-red-700 hover:to-purple-700 transition-all duration-300 font-semibold">
        ...
        @if ($featuredReviews->count() > 0)
            <div class="max-w-6xl mx-auto px-4 mb-16">
                <h2
                    class="text-4xl font-bold text-center mb-12 text-transparent bg-clip-text bg-gradient-to-r from-red-400 to-purple-600">
                    ⭐ Em Destaque
                </h2>
                <div class="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
                    @foreach ($featuredReviews as $review)
                        <div
                            class="card-hover bg-gradient-to-br from-gray-800 to-gray-900 rounded-lg overflow-hidden shadow-2xl border border-red-900">
                            @if ($review->image_url)
                                <div class="h-48 bg-cover bg-center"
                                    style="background-image: url('{{ $review->image_url }}')"></div>
                            @endif
                            <div class="p-6">
        ...
        <div class="max-w-6xl mx-auto px-4 pb-16">
            <h2
                class="text-4xl font-bold text-center mb-12 text-transparent bg-clip-text bg-gradient-to-r from-purple-400 to-red-600">
            ...
            @if ($reviews->count() > 0)
                <div class="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
                    @foreach ($reviews as $review)
                        <div
                            class="card-hover bg-gradient-to-br from-gray-800 to-gray-900 rounded-lg overflow-hidden shadow-2xl border border-gray-700">
                            @if ($review->image_url)
                                <div class="h-48 bg-cover bg-center"
                                    style="background-image: url('{{ $review->image_url }}')"></div>
                            @endif
